...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const ImRect bb(pos + ImVec2(7, 0), pos + size);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = 6;
    const ImVec2 pos = window->DC.CursorPos + ImVec2(7, 4);
    const ImRect total_bb(pos, pos + ImVec2(square_sz + 5 + label_size.x, (label_size.y > square_sz) ? label_size.y : square_sz));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(25, 25, 25) , 1.f);

    if (*v) {
        window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), GetColorU32(ImGuiCol_CheckMark), 1.f);
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f));
    }
    else
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(115 / 255.f, 115 / 255.f, 115 / 255.f, 1.f));

    window->DrawList->AddRect(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), GetColorU32(ImGuiCol_BorderShadow), 1.f);

    RenderText({ total_bb.Max.x - label_size.x, total_bb.Min.y - 4 }, label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(7, 0), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
...
//Combo box allowing to select multiple data
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(7, 0), window->DC.CursorPos + ImVec2(2 + w, 4));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(25, 25, 25), 1.f);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(27, 27, 27), 1.f);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2((frame_bb.Min.x + 14, grab_bb.Max.x), frame_bb.Max.y), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), 1.f);

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255)).operator ImVec4());
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (ButtonEx("-", ImVec2(button_size + 5, button_size), button_flags))
        {
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(5, 0), window->DC.CursorPos + frame_size);
...
void ImGui::ColorEdit5( const char* label, float* col ) {
    ...
    ImGui::ColorEdit4( label, col, ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoOptions );
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, ImVec2(18, 11)))
        {
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float bars_width = 9; // Arbitrary smallish width of Hue/Alpha picking bars
    float sv_picker_size = 195; // ImMax( bars_width * 1, CalcItemWidth( ) - ( alpha_bar ? 2 : 1 ) * ( bars_width + style.ItemInnerSpacing.x ) ); // Saturation/Value picking box
   ...
   //draw_list->AddRectFilled(picker_pos - ImVec2(300, 16), picker_pos + ImVec2(1500, 500), ImColor(0.f, 0.f, 0.f, 1.f));
    ...
    if (flags & ImGuiColorEditFlags_PickerHueBar) {
        ...
        draw_list->AddRectFilled(ImVec2(bar0_pos_x + 0, bar0_line_y + 1), ImVec2(bar0_pos_x + bars_width - 1, bar0_line_y + 3), IM_COL32(255, 255, 255, 255));
    ...
    if (alpha_bar) {
        ...
        draw_list->AddRectFilled(ImVec2(bar1_pos_x + 0, bar1_line_y + 1), ImVec2(bar1_pos_x + bars_width - 1, bar1_line_y + 3), IM_COL32(255, 255, 255, 255));
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (held && (flags & ImGuiSelectableFlags_DrawHoveredWhenHeld))
        hovered = true;
    ImColor col = ((selected && hovered) ? ImColor(170, 170, 170, 200) : hovered ? ImColor(menu::menu_accent[0], menu::menu_accent[1], menu::menu_accent[2], 0.39216) : ImColor(menu::menu_accent[0], menu::menu_accent[1], menu::menu_accent[2]));
    if (hovered || selected)
    {
        RenderFrame(bb.Min, ImVec2(bb.Min.x + 2, bb.Max.y), col, false, 0.0f);
    ...
    ImVec4 t_col = ((selected && hovered) ? ImVec4(170 / 255.f, 170 / 255.f, 170 / 255.f, 1.f) : hovered ? ImVec4(177 / 255.f, 177 / 255.f, 177 / 255.f, 0.7f) : selected ? ImVec4(menu::menu_accent[0], menu::menu_accent[1], menu::menu_accent[2], 1.f) : ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f));

    PushFont(selected ? fonts::menu_font_bold : fonts::menu_font_thin);

    if (hovered || selected) {
        PushStyleColor(ImGuiCol_Text, ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 1.f));
        RenderText(text_min + ImVec2(6, 1), label);
        PopStyleColor();
    }

    PushStyleColor(ImGuiCol_Text, t_col);

    RenderText(text_min + ImVec2(5, 0), label);
