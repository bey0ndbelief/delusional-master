...
namespace math {
	...
	inline vec3_t og_angel;
	inline float og_forwar_move, og_side_move;
	inline void start_movement_fix(c_usercmd* cmd) {
		og_angel = cmd->view_angles;
		og_forwar_move = cmd->forward_move;
		og_side_move = cmd->side_move;
	}

	inline void end_movement_fix(c_usercmd* cmd) {
		float deltaViewAngles;
		float f1;
		float f2;

		if (og_angel.y < 0.f)
			f1 = 360.0f + og_angel.y;
		else
			f1 = og_angel.y;
		...
		cmd->forward_move = cos(deg2rad(deltaViewAngles)) * og_forwar_move + cos(deg2rad(deltaViewAngles + 90.f)) * og_side_move;
		cmd->side_move = sin(deg2rad(deltaViewAngles)) * og_forwar_move + sin(deg2rad(deltaViewAngles + 90.f)) * og_side_move;
	}
