...
public:
	...
	void drawpolyline(const std::vector<ImVec2>& verts, const color_t& color, const ImDrawFlags flags = 0, const float thickness = 1.0f);
	int get_text_width(const char* text, ImFont* font);
	ImVec2 get_text_size(ImFont* font, const std::string& text);
...
namespace fonts {
	...
	inline std::string font_directory_watermark = "C:/windows/fonts/tahomabd.ttf";
