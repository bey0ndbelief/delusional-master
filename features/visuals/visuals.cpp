...
void features::visuals::trail() {
	...
	auto pLocal = reinterpret_cast<player_t*>(interfaces::ent_list->get_client_entity(interfaces::engine->get_local_player()));
	if (!pLocal)
		return;

	vec3_t LocalOrigin = g::local->origin();
	int LocalVelocity = pLocal->velocity().length_2d();

	if (!(pLocal->flags() & fl_onground) && tick == 0) {

		int red, green, blue, alpha;

		red = c::visuals::trails_clr1[0] * 255;
		green = c::visuals::trails_clr1[1] * 255;
		blue = c::visuals::trails_clr1[2] * 255;
		alpha = c::visuals::trails_clr1[3] * 255;
		...
		BeamInfo.m_flBrightness = alpha;
		BeamInfo.m_flSpeed = 0.5;
		BeamInfo.m_nStartFrame = 0.;
		BeamInfo.m_flFrameRate = 0.;
		BeamInfo.m_flRed = red;
		BeamInfo.m_flGreen = green;
		BeamInfo.m_flBlue = blue;
		...
		Beam_t* myBeam = interfaces::render_beams->create_beam_point(BeamInfo);
