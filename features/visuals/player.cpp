...
void features::visuals::player::draw_name(player_t* entity, bbox_t bbox) {
	if (c::visuals::playername) {
		...
		im_render.text(bbox.x + (bbox.w / 2), bbox.y - 13, c::fonts::esp_name_size, fonts::esp_name_font, print, true,
...
void features::visuals::player::draw_health(player_t* entity, bbox_t bbox) {
	if (c::visuals::healthesp) {
		...
		if (c::visuals::healthesp_s == 0) {
			...
			if (entity->health() < 100)
				im_render.text(bbox.x - 15, bbox.y - 1 + (bbox.h - bbox.h * (prev_player_hp[entity->index()]) / 100.f) - 3, c::fonts::esp_hp_size, fonts::esp_hp_font, std::to_string(static_cast<int>(prev_player_hp[entity->index()])), true,
			...
			//text
			im_render.text(bbox.x + (bbox.w / 2), bbox.y + bbox.h + (c::visuals::playerarmor ? 4 : 1), c::fonts::esp_hp_size, fonts::esp_hp_font, healtht, true,
				color_t(c::visuals::health_color[0], c::visuals::health_color[1], c::visuals::health_color[2]).get_color_edit_alpha_int(fade.at(entity->index())), c::fonts::esp_font_flag[9], c::fonts::esp_font_flag[10]);
			offset += 13;
		}
		else if (c::visuals::healthesp_s == 2) {
			//only text
			im_render.text(bbox.x - 13, bbox.y - 1 + (bbox.h - bbox.h * (prev_player_hp[entity->index()]) / 100.f) - 3, c::fonts::esp_hp_size, fonts::esp_hp_font, std::to_string(static_cast<int>(prev_player_hp[entity->index()])), true,
				color_t(c::visuals::health_color[0], c::visuals::health_color[1], c::visuals::health_color[2]).get_color_edit_alpha_int(fade.at(entity->index())), c::fonts::esp_font_flag[9], c::fonts::esp_font_flag[10]);
		}
	}
}
...
void features::visuals::player::flags(player_t* entity, bbox_t bbox) {
	...
	auto position = 0;
	for (auto text : flags) {
		im_render.text(bbox.x + bbox.w + 3, bbox.y + position - 2, 11.f, fonts::esp_misc, text.first, false, text.second, true);
		position += 10;
	}
}
...
void features::visuals::dropped_weapons(player_t* entity) {
	...
	if (get_playerbox(weapon, bbox) && strstr(model_name, "models/weapons/w_") && strstr(model_name, "_dropped.mdl")) {
		...
		if (c::visuals::dropped_weapon_icon) {
			...
			im_render.text(bbox.x + (bbox.w / 2), bbox.y + 7, 11.f, fonts::esp_wpn_icon, wep_name, true,
...
void features::visuals::player::backtrack_dot(player_t* entity) {
	...
	for (int i = 1; i < backtrack.data[entity->index()].size(); i++) {//loop bt records
		...
		if (previous_screenpos.is_valid() && !previous_screenpos.is_zero()) {
			...
			im_render.drawrect(screen_pos.x, screen_pos.y, 2, 2, color_t(c::visuals::skeletonbt_clr[0], c::visuals::skeletonbt_clr[1], c::visuals::skeletonbt_clr[2]).get_color_edit_alpha_int(fade.at(entity->index())));
